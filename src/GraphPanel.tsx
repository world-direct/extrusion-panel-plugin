...
class GraphPanel extends React.Component<Props, State> {
    ...
    onExportClick = () => {
        ...
        if (anyGraphJson && Array.isArray(anyGraphJson.values)) {
            anyGraphJson.values.forEach((value: { value?: number; timestamp?: number; latitude?: number; longitude?: number; serial?: string; }) => {
                if (value.value && value.timestamp) {
                    bytes.push(value.value + '; ' + moment(value.timestamp).format("DD-MM-YYYY hh:mm:ss") + '; ' + value.latitude + '; ' + value.longitude + '; ' + value.serial + '\n');
                }
            });
        }
